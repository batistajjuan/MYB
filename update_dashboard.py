...
def parse_sql_output(content, delimiter=';'):
    ...
    return (monthly, suc_monthly, vend_monthly, vend_inv, vend_cl, cl_total,
            ...
            prod_sales, prod_margin, prod_qty)
...
def build_retention(cl_total):
    ...
    common = [(c, int(cl_total[2026][c]), int(cl_total[2025][c]))
              for c in retained]
    ...
    top_new = sorted([(c, int(cl_total[2026][c])) for c in new26],
                     key=lambda x: -x[1])[:10]
...
def main():
    ...
    (monthly_raw, suc_monthly, vend_monthly, vend_inv, vend_cl, cl_total,
     margin_monthly, vend_margin, suc_margin,
     prod_sales, prod_margin, prod_qty) = result
    ...
    retention = build_retention(cl_total)
