...
def stable_client_key(cl_code, cl_name):
    ...
    # Código válido = tiene al menos un dígito no-cero
    if code and re.search(r'[1-9]', code):
        return 'C:' + code
    # Fallback: normalizar nombre
    name = cl_name.strip().upper()
    name = re.sub(r'[.,;\-\'\"()]', ' ', name)       # quitar puntuación
    name = _LEGAL_RE.sub('', name)                    # quitar sufijo legal
    name = re.sub(r'\b(?:SRL|SA|EIRL|CXA|SAS)\b', '', name)  # sufijos restantes
...
def parse_sql_output(content, delimiter=';'):
    ...
    for row in reader:
        try:
            ...
            # Stable key: code (if non-trivial) or normalized name fallback
    ...
    # ── Diagnóstico cl_total (para debug retención) ────────────────────────────
    for yr in [2025, 2026]:
        n_cl = len(cl_total[yr])
        sample = list(cl_total[yr].keys())[:3]
        print(f"    cl_total[{yr}]: {n_cl} clientes únicos | primeras claves: {sample}")
    print(f"    cl_names: {len(cl_names)} entradas | tiene_CLIENTE_col: {'CLIENTE' in col}")
