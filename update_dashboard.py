...
def get_logical_names(bak_path):
    ...
    for line in out.splitlines():
        ...
        if len(cols) >= 2 and cols[1] in ('D', 'L') and cols[0] and cols[0] != 'LogicalName':
            if cols[1] == 'D' and not data_name:
                data_name = cols[0]
            elif cols[1] == 'L' and not log_name:
                log_name = cols[0]
...
def run_bak_mode():
    ...
    print("[1/5] Descargando base de datos .bak...")
    download_file(bak_url, BAK_PATH)
...
def build_monthly(monthly):
    result = {}
    for yr in [2025, 2026]:
        arr = [int(monthly[yr].get(mo, 0)) for mo in range(1, 7)]
        if yr == 2025:
            arr[0] = JAN_2025_TOTAL   # histórico hardcodeado
        result[yr] = arr
    return result

def build_sucs(suc_monthly):
    sucs = []
    for key in SUC_ORDER:
        name, short, cls = SUC_DEF[key]
        m25 = [int(suc_monthly[key][2025].get(mo, 0)) for mo in range(1, 7)]
        m26 = [int(suc_monthly[key][2026].get(mo, 0)) for mo in range(1, 7)]
...
def build_vendors(vend_monthly, vend_inv, vend_cl):
    vendors = {}
    for yr in [2025, 2026]:
        ...
        for vend, months in ranked:
            ...
            lst.append({
                ...
                'm':  [int(months.get(mo, 0)) for mo in range(1, 7)],
...
def main():
    ...
    sucs     = build_sucs(suc_monthly)
    vendors  = build_vendors(vend_monthly, vend_inv, vend_cl)
