...
def parse_sql_output(content, delimiter=';'):
    ...
    # Diagnóstico: mostrar qué meses tienen datos por año
    for yr in [2025, 2026]:
...
def render_js(md, sucs, vendors, top_cl):
    ...
    # Usar el último año con datos, y el último mes con valor >0 en ese año
    last_yr = 2026 if any(md[2026]) else 2025
    last_yr_arr = md[last_yr]
    last_mo_idx = max((i for i, v in enumerate(last_yr_arr) if v > 0), default=max_mo - 1)
    last_mo_label = MONTH_LABELS_ES[last_mo_idx]
